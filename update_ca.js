...
const PREVIEW_FILENAME = `site_preview_${previewTimestamp}.png`;
...
function takeScreenshot() {
    ...
    const outputPath = path.join(PROJECT_DIR, PREVIEW_FILENAME);
    
    // Cleanup old preview files before taking the new one
    try {
        const files = fs.readdirSync(PROJECT_DIR);
        for (const file of files) {
            if (file.startsWith('site_preview') && file.endsWith('.png')) {
                fs.unlinkSync(path.join(PROJECT_DIR, file));
            }
        }
    } catch(err) {
        console.error('    ⚠️  Could not clean up old preview images:', err.message);
    }
    ...
    try {
        ...
        console.log(`✅  Screenshot saved to ${PREVIEW_FILENAME}`);
    } catch (err) {
...
async function main() {
    ...
    html = html.replace(/(<meta property="og:image" content=")[^"]*(")/gi, `$1${BASE_URL}${PREVIEW_FILENAME}$2`);
    ...
    html = html.replace(/(<meta (?:property|name)="twitter:image" content=")[^"]*(")/gi, `$1${BASE_URL}${PREVIEW_FILENAME}$2`);
